...
const getProductHistoriesByIdHistory = async (request, response) => {
  ...
  if (idHistory) {
    ...
    return showApi.showResponse(response, 'Id History must be filled', null, 400);
  }
};
    ...
    if (!isNaN(id)) {
      ...
      if (getDataProductHistory.length > 0) {
        ...
        if (result.affectedRows > 0) {
          ...
          showApi.showResponse(response, 'Data product history failed to delete!', null, 500);
        }
      } else {
        showApi.showResponse(response, 'Data product history not found!', null, 404);
      }
    } else {
      return showApi.showResponse(response, 'Id must be a number', null, null, 400);
    }
