...
// const getFilterDelivery = async (request, response) => {
...
//   const dataProduct = await productModel.getFilterDeliveryMethodProducts(data)

//   if (dataProduct.length > 0) {
//     const result = await productModel.countFilterDeliveryMethodProducts(data)
//     try {
...
const getFilter = async (request, response) => {
  ...
  if (errValidation == null) {
    ...
    console.log(offset)
    const data = { name, filter, limit, offset, sort, order }
    const dataSize = await productModel.getFilterSizeProducts(data)
    // const dataProduct = await productModel.getFilterDeliveryMethodProducts(data)

    if (dataSize.length > 0) {
      const result = await productModel.countFilterSizeProducts(data)
      try {
        const { total } = result[0]
        pagination = { ...pagination, total: total, route: route }
        return showApi.showResponseWithPagination(response, 'List Data Product', dataSize, pagination)
      } catch (err) {
        return showApi.showResponse(response, err.message, null, 500)
      }
    } else {
...
module.exports = { getProducts, getProduct, insertProduct, updateProduct, updatePatchProduct, deleteProduct, getFilter /* getFilterDelivery */ }
