...
const getPromotionSize = async (request, response) => {
  ...
  if (result.length > 0) {
    ...
    return showApi.showResponse(response, 'Detail promotion size not found!', null, 404);
  }
};
      ...
      if (dataPromotionSize.length > 0) {
        ...
        if (errValidation === null) {
          ...
          if (success) {
            ...
            showApi.showResponse(response, 'Data promotion size failed to update!', 500);
          }
        } else {
          showApi.showResponse(response, 'Data promotion size not valid', null, errValidation, 400);
        }
...
const deletePromotionSize = async (request, response) => {
  ...
  if (getDataPromotionSize.length > 0) {
    ...
    showApi.showResponse(response, 'Data promotion size not found!', null, 404);
  }
};
